  ...
  return (
    <div className="px-6 py-5">
      <div className="mb-5">
        ...
        <p className="text-sm text-muted-foreground">Manage your profile, team membership, and cadence.</p>
      </div>
...
function ProfileTab({ userId }: { userId: string }) {
  ...
  async function save() {
    ...
    const { error } = await supabase
      .from("profiles")
      .update({ display_name: displayName || null, avatar_url: avatarUrl || null, role_label: roleLabel || null })
      .eq("id", userId);
    setBusy(false);
  ...
  if (isLoading) {
    return <div className="grid place-items-center py-12 text-muted-foreground"><Loader2 className="h-5 w-5 animate-spin" /></div>;
  }
  ...
  return (
    <section className="panel max-w-xl space-y-4 p-6">
      ...
      <div className="space-y-1.5">
        ...
        <Input value={avatarUrl} onChange={(e) => setAvatarUrl(e.target.value)} className="rounded-xl" placeholder="https://…" />
      </div>
      <div className="space-y-1.5">
        <Label>Role label</Label>
        <Input value={roleLabel} onChange={(e) => setRoleLabel(e.target.value)} className="rounded-xl" placeholder="Founder, Operator…" />
      </div>
      <div className="flex justify-end">
        <Button onClick={save} disabled={busy} className="bg-iris text-white">{busy ? "Saving…" : "Save profile"}</Button>
      </div>
    </section>
  );
}
  ...
  return (
    <div className="space-y-5">
      {isAdmin && (
        <section className="panel max-w-xl space-y-4 p-6">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Sends an email invitation.</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label>Email</Label>
              <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@company.com" className="rounded-xl" />
            </div>
            <div className="space-y-1.5">
      ...
      <section className="panel p-0">
        <div className="border-b border-border px-5 py-3"><h2 className="text-sm font-semibold">Team members</h2></div>
        {isLoading ? (
              ...
              <li key={m.id} className="flex items-center justify-between gap-3 px-5 py-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="truncate text-xs text-muted-foreground">{m.role_label ?? "—"}</div>
                  </div>
                </div>
                <div className="flex flex-wrap gap-1">
                  {m.roles.length === 0 && <Chip tone="muted">no role</Chip>}
                  {m.roles.map((r) => (
                    <Chip key={r} tone={r === "admin" ? "iris" : "neutral"}>{r}</Chip>
                  ))}
                </div>
              </li>
            ))}
            {(data ?? []).length === 0 && (
              <li className="py-10 text-center text-sm text-muted-foreground">No team members yet.</li>
            )}
          </ul>
        )}
      </section>
